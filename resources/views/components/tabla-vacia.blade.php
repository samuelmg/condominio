...
<div class="flex items-center mt-6 text-center border rounded-lg h-96 dark:border-gray-700">
    <div class="flex flex-col w-full max-w-sm px-4 mx-auto">
        ...
        <h1 class="mt-3 text-lg text-gray-800 dark:text-white">{{ $titulo }}</h1>
        <p class="mt-2 text-gray-500 dark:text-gray-400">{{ $mensaje }}</p>
        <div class="flex items-center mt-4 sm:mx-auto gap-x-3">
            ...
            @if(!empty($ruta))
            <a href="{{ route('vivienda.create') }}" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 bg-blue-500 rounded-lg shrink-0 sm:w-auto gap-x-2 hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
